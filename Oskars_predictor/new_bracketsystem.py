...
import joblib
import csv
from refined_prediction import simulate_match_with_draw_boost
...
with open('Oskars_predictor/full_simulation3.csv', 'w', newline='') as csvfile:
...
def update_team_progress(team_name, stage):
    # Read current data
    with open('Oskars_predictor/full_simulation3.csv', 'r') as csvfile:
...
def predict_match_result(home_team, away_team, group_stage):
    ...
    home_win_prob, draw_prob, away_win_prob = simulate_match_with_draw_boost(home_team, away_team, 'Germany', home_ranking, away_ranking, home_avg_goals_scored, away_avg_goals_scored, home_avg_goals_conceded, away_avg_goals_conceded, home_win, away_win, home_loss, away_loss, home_draw, away_draw)
